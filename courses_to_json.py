...
SAP_BASE_URL = "https://portalex.technion.ac.il/sap/opu/odata/sap/Z_CM_EV_CDIR_DATA_SRV/"
...
def send_request_once(query: str, allow_empty: bool):
    ...
    if response.status_code != 200:
        raise RuntimeError(f"Bad status code: {response.status_code}, expected 200")
...
def get_sap_course(year: int, semester: int, course: str):
    ...
    raw_data = send_request(
        f"SmObjectSet(Otjid='{course}',Peryr='{year}',Perid='{semester}',ZzCgOtjid='',ZzPoVersion='',ZzScOtjid='')?{urllib.parse.urlencode(params)}"
    )
    return raw_data["d"]
